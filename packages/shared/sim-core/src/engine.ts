import { slippagePreview } from "./slippage.js";
import type {
  MarketId,
  Position,
  PriceTick,
 ...
 * (ids, timestamps, persistence) lives in the caller; this module is the math.
 *
 * Shared verbatim between the trader app and the executor service so the live
 * overlay and the authoritative on-chain settlement never disagree on a number.
 *
...
export function applyFill(
  ...
  const p = slippagePreview({ marketId, side, sizeUsd, oracleMid, depthUsd });
  return {
    fill: p.fill,
...
/** Re-mark every open position against the latest prices. */
 ...
 * close live in the caller's recompute.
 */
 ...
 * canonical rule computation — the mock layer's `buildRuleBudgets` delegates
