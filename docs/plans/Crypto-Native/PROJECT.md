...
A Sui-based paper-trading evaluation platform where:

- Invited traders connect a Sui wallet, pick a tier, and paper-trade against **live mainnet prices** with a calibrated slippage and fill model.
...
- **Primary:** 50–100 invited crypto traders sourced from the founder's existing networks (Octant, Bungee, study groups, existing crypto trading communities). Already crypto-native, already trade, already have wallets.
...
- **Chain:** Sui Move. Per-trader owned objects naturally model evaluation vaults; resource semantics give strong enforcement guarantees.
...
| Smart contract engineer | 1 person | Sui Move contracts, SBT, enforcement |
...
- **Shadow-quote source on Sui = 7K Protocol aggregator API.** Single integration; routes across Cetus, Aftermath, Turbos, Kriya; tracks realistic aggregator-quality fills that a real trader would experience.
- **Operational tooling:**
  ...
  - **Sui native MultiSig** as the upgrade authority for Sui packages (2/3 multisig: founder + SC engineer + operator).
...
- **Oracle staleness thresholds:** Sui revert if Pyth price age > 10s (threshold accounts for Wormhole VAA posting latency).
...
- **Trade-intent rate limit:** `min_interval` = 2s; `MAX_INTENTS` = 200 per evaluation. Per-vault state holds `last_intent_at` + `intent_count`; vault reverts `RATE_LIMITED` or `INTENT_CAP` accordingly. Blocks HFT/latency-arbitrage strategies; covers swing + discretionary. Tunable post-beta.

### Round 3 (Tier-3 — engagement narrative + post-launch behavior)

- **Trader profile URLs:** `/trader/sui/<addr>`. Each wallet has its own profile page tied to the Sui address.
- **v1 SBT → v2 transition = in-place evolution.** Same SBT object across v1 and v2. v1 fields (`highest_tier`, `total_passes`, `total_shadow_pnl`, `total_trades`, `last_active_at`) stay; v2 adds (`real_capital_passes`, `real_pnl`, `funded_at`, `payout_history`). The v1 cohort SBT is the genesis credential — never re-issued, never transferable, remains top-tier identity primitive in v2. **Soft-airdrop page copy reflects this without using token/airdrop language.**
