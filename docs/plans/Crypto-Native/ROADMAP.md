...
Phase 0 → Phase 1 ──────────────────┐
       ...
       ↘ Phase 1 ┘                  │
                  ...
                  Phase 5 (∥ 4) ────┘
```
Phases 1/2/3 overlap heavily (wk 1–9). Phases 4 (Solana port) and 5 (Engagement) run in **parallel** (wk 7–12). Phase 7 ends with beta open.
...
### Phase 4: Solana Port [ ]
**Goal:** All Phase 1 modules ported to Anchor with PDA seeds, paginated TradeLog, Sealevel-attacks negative tests.
**Tasks:** ~8 atomic. **Depends on:** Phase 1 API frozen. **Requirements:** [REQ-03, REQ-06, REQ-07]
**Verification:** `anchor test` (bankrun) green; cross-chain parity diff = 0 for identical inputs; constraint checklist signed off; Helius LaserStream ingest live.

### Phase 5: Engagement Layer [ ] *(parallel with Phase 4)*
**Goal:** Public leaderboard + per-chain profile pages + SBT level-up + `/v1-cohort` page with soft-airdrop messaging stance.
**Tasks:** ~7 atomic. **Depends on:** Phase 2 (indexer) + designer SBT art. **Requirements:** [REQ-04, REQ-11]
**Verification:** SBTs render in Suiet + Phantom; `/v1-cohort` page contains zero token/airdrop language (CI lint); leaderboard sorts deterministically.
...
**Tasks:** ~6 atomic. **Depends on:** Phases 2 + 3 + 4. **Requirements:** [REQ-01, REQ-06]
...
| REQ-03 | Sui + Solana feature parity at v1 launch | PROJECT.md Success Criteria |
...
| REQ-08 | Cross-chain event schema parity via codegen + CI gate | STATE.md + PITFALLS 2.11 |
| REQ-09 | Multisig upgrade authority on both chains from day 1 | Clarify R1 + PITFALLS 4.8 |
...
- [ ] Sui + Solana ship at parity at launch
...
| Cross-chain event schema drift | Codegen + CI parity gate | 0 |
